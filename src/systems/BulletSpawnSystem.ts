...
export const createBulletSpawnSystem = () => {
  ...
  const bulletSpawnExitQuery = exitQuery(bulletSpawnQuery);
  ...
  return (world: World) => {
    ...
    for (const entity of bulletSpawnExitQuery(world)) {
      configManager.remove(BulletSpawnComponent.configIndex[entity]);
    }
